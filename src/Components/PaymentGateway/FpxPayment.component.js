...
const FpxPayment = ({ productInfo, handleBackButton }) => {
    ...
    useEffect(() => {
        const getClientSecret = async () => {
            const { data } = await axios.post(
                `${process.env.REACT_APP_STRIPE_ENDPOINT}/fpx`,
                {
    ...
    const handleSubmit = async (event) => {
        ...
        const { error, paymentIntent } = await stripe.confirmFpxPayment(
            clientSecret,
            {
                ...
                return_url: `${window.location.origin}/iap?product_info=${productInfo?.id}&qty=${productInfo?.quantity}&return_fpx=true`,
            }
        );
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <FpxBankElement
                                                ...
                                                onReady={() => {
                                                    console.log(
                                                        "FpxBankElement [ready]"
                                                    );
                                                }}
                                                onChange={(event) => {
                                                    console.log(
                                                        "FpxBankElement [change]",
                                                        event
                                                    );
                                                }}
                                                onBlur={() => {
