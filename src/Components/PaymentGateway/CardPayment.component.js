...
const CardPayment = ({ productInfo, handleBackButton }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (productInfo?.type === "subscription") {
            const payMethodResponse = await stripe.createPaymentMethod({
                type: "card",
            ...
            if (payMethodResponse.error) {
                ...
                const subInfoResponse = await axios.post(
                    `${process.env.REACT_APP_STRIPE_ENDPOINT}/sub`,
                    {
                        paymentMethod: payMethodResponse.paymentMethod.id,
                        email: email,
                        productName: productInfo?.details?.productName,
                        productType: productInfo?.details?.productType,
                        planInterval: productInfo?.details?.planInterval,
                        planIntervalCount:
                            productInfo?.details?.planIntervalCount,
                        price: Math.ceil(productInfo?.details?.price * 100),
                    }
                );
                const { subscription } = subInfoResponse.data;
                if (subscription.id) {
